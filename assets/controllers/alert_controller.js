...
export default class extends Controller {
    ...
    delete_saml() {
        ...
        if (
            confirm(
                `Are you sure you want to delete this SAML Provider "${saml_name}"? All the users associated with this provider will also be eliminated`,
            )
        ) {
            // If confirmed, submit the form
            this.formTarget.submit();
    ...
    revoke_saml() {
        const providerName = this.buttonTarget.getAttribute("data-provider-name");
        if (
            confirm(`Are you sure you want to revoke all profiles associated with the SAML Provider: ${providerName}?`)
        ) {
            // If confirmed, submit the form
            this.formTarget.submit();
        }
