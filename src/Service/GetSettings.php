...
class GetSettings
{
    public function getSettings(UserRepository $userRepository, SettingRepository $settingRepository): array
    {
        ...
        $data['code'] = [
            'value' =>
                ($user = $userRepository->findOneBy(['twoFAcode' => null]))
                    ? $user->getTwoFAcode()
                    : null,
    ...
    public function getSettingDescription($settingName): string
    {
        $descriptions = [
            ...
            'LOGIN_WITH_UUID_ONLY' => 'Defines whether authentication is performed using a link sent to the user or a 
            tradicional uuid & password.',
