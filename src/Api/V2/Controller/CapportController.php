...
{
    public function __construct(private readonly \App\Repository\SettingRepository $settingRepository)
    {
    }
    #[Route('/capport/json', name: 'api_v2_capport_json', methods: ['GET'])]
    public function capportJson(): JsonResponse
    {
        if ($this->settingRepository->findOneBy(['name' => SettingName::CAPPORT_ENABLED->value])->getValue() !== 'true') {
