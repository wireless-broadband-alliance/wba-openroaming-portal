...
class TwoFAController extends AbstractController
{
    ...
    public function twoFARequest(Request $request): JsonResponse
    {
        ...
        $turnstileSetting = $this->settingRepository->findOneBy(['name' => SettingName::TURNSTILE_CHECKER->value])->getValue();
        if (!$turnstileSetting) {
        ...
        $timeToResendIntervalValue = $this->settingRepository->findOneBy(['name' => SettingName::TWO_FACTOR_AUTH_RESEND_INTERVAL->value]);
