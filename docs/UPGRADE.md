...
| 1.10.0          | 1.11.0         | Run `php bin/console doctrine:migrations:migrate`. After upgrading, run `php bin/console reset:convert-admin-roles` **once** to promote the admin user to **Super Admin** for the new role hierarchy system. Note this command it's only present on this release for the migration. |
